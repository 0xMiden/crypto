...
//! cargo run -p p3-miden-bench --features parallel --release
//!
//! # Custom traces:
//! cargo run -p p3-miden-bench --features parallel --release -- keccak:15 keccak:18 keccak:19
//!
//! # Full tracing tree:
//! cargo run -p p3-miden-bench --features parallel --release -- -v keccak:15
//!
//! # Multi-iteration with warm-up (reports min/median/mean/max):
//! cargo run -p p3-miden-bench --features parallel --release -- -n 5 keccak:15
//!
//! # Miden-shaped AIR (auto log_blowup=3):
//! cargo run -p p3-miden-bench --features parallel --release -- miden:18:51 miden:19:20
//!
//! # Batch-STARK comparison:
//! cargo run -p p3-miden-bench --features parallel --release -- -m batch keccak:15 keccak:18
//! ```
...
type BatchPoseidon2Air = p3_poseidon2_air::Poseidon2Air<
    ...
    WIDTH,
    { p3_miden_lifted_stark::testing::airs::poseidon2::SBOX_DEGREE },
    { p3_miden_lifted_stark::testing::airs::poseidon2::SBOX_REGISTERS },
    HALF_FULL_ROUNDS,
    PARTIAL_ROUNDS,
>;
