...
proptest! {
    ...
    fn test_encrypted_data_serialization_roundtrip(
        ...
        let encrypted = key.encrypt_felts_with_nonce(&data, &associated_data, nonce).unwrap();
    ...
    fn test_encryption_decryption_roundtrip(
        ...
        let encrypted = key.encrypt_felts_with_nonce(&data, &associated_data, nonce).unwrap();
        let decrypted = key.decrypt_felts_with_associated_data(&encrypted, &associated_data).unwrap();
    ...
    fn test_different_keys_different_outputs(
        ...
        let encrypted1 = key1.encrypt_felts_with_nonce(&data, &associated_data, nonce1).unwrap();
        let encrypted2 = key2.encrypt_felts_with_nonce(&data, &associated_data, nonce2).unwrap();
    ...
    fn test_different_nonces_different_outputs(
        ...
        let encrypted1 = key.encrypt_felts_with_nonce(&data,&associated_data, nonce1).unwrap();
        let encrypted2 = key.encrypt_felts_with_nonce(&data, &associated_data, nonce2).unwrap();
...
fn test_empty_data_encryption() {
    ...
    let encrypted = key.encrypt_felts_with_nonce(&empty_data, &associated_data, nonce).unwrap();
    let decrypted = key.decrypt_felts_with_associated_data(&encrypted, &associated_data).unwrap();
...
fn test_encryption_various_lengths() {
    ...
    for len in [1, 7, 8, 9, 15, 16, 17, 31, 32, 35, 39, 54, 67, 100, 1000] {
        ...
        let encrypted = key.encrypt_felts_with_nonce(&data, &associated_data, nonce).unwrap();
        let decrypted =
            key.decrypt_felts_with_associated_data(&encrypted, &associated_data).unwrap();
...
fn test_ciphertext_tampering_detection() {
    ...
    let mut encrypted = key.encrypt_felts_with_nonce(&data, &associated_data, nonce).unwrap();

    // Tamper with ciphertext
    encrypted.ciphertext[0] += ONE;

    let result = key.decrypt_felts_with_associated_data(&encrypted, &associated_data);
...
fn test_wrong_key_detection() {
    ...
    let associated_data: Vec<Felt> = vec![ONE; 8];
    let data = vec![Felt::new(123), Felt::new(456)];
    let encrypted = key1.encrypt_felts_with_nonce(&data, &associated_data, nonce).unwrap();

    // Try to decrypt with wrong key
    let result = key2.decrypt_felts_with_associated_data(&encrypted, &associated_data);
    assert!(matches!(result, Err(EncryptionError::InvalidAuthTag)));
}
...
fn test_wrong_nonce_detection() {
    ...
    let associated_data: Vec<Felt> = vec![ONE; 8];
    let data = vec![Felt::new(123), Felt::new(456)];
    let mut encrypted = key.encrypt_felts_with_nonce(&data, &associated_data, nonce1).unwrap();
...
mod security_tests {
    ...
    fn test_ciphertext_appears_random() {
        ...
        for _ in 0..100 {
            let nonce = Nonce::with_rng(&mut rng);
            let encrypted =
                key.encrypt_felts_with_nonce(&plaintext, &associated_data, nonce).unwrap();
