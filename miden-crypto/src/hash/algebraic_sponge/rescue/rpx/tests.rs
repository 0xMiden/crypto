...
mod p3_tests {
    ...
    /// `impl<P: PackedValue<Value = Felt>> Permutation<[P; STATE_WIDTH]> for RpxPermutation256`
    /// is bit-exact equivalent to running the scalar permutation on each
    /// lane independently. See the equivalent rpo test for design rationale.
    #[test]
    fn test_rpx_permutation_packed_lane_equivalence() {
        ...
        let mut ref0 = lane0;
        let mut ref1 = lane1;
        RpxPermutation256::apply_permutation(&mut ref0);
        RpxPermutation256::apply_permutation(&mut ref1);

        let mut packed: [Packed2; STATE_WIDTH] =
            core::array::from_fn(|i| Packed2([lane0[i], lane1[i]]));
        RpxPermutation256.permute_mut(&mut packed);
