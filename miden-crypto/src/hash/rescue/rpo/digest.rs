...
impl TryFrom<RpoDigest> for [bool; DIGEST_SIZE] {
    ...
    fn try_from(value: RpoDigest) -> Result<Self, Self::Error> {
        ...
        Ok([
            to_bool(value.0[0].as_canonical_u64()).ok_or(RpoDigestError::TypeConversion("bool"))?,
            to_bool(value.0[1].as_canonical_u64()).ok_or(RpoDigestError::TypeConversion("bool"))?,
            to_bool(value.0[2].as_canonical_u64()).ok_or(RpoDigestError::TypeConversion("bool"))?,
            to_bool(value.0[3].as_canonical_u64()).ok_or(RpoDigestError::TypeConversion("bool"))?,
        ])
    }
}
