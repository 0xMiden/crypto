//! This module contains the iterators needed by the persistent backend of the forest.
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match &mut self.state {
            PersistentBackendEntriesIteratorState::NotInLeaf => {
                // Here we are not in a leaf of the targeted tree, so we have to see if we _can_ be.
                if let Some(entry) = self.iterator.next() {
                    let (key_bytes, value_bytes) = entry.ok()?;
                    let key = LeafKey::read_from_bytes(&key_bytes).ok()?;

                    // If the key isn't for the correct lineage (which can happen even with the
                    // bloom filter), we need to advance.
            ...
            PersistentBackendEntriesIteratorState::InLeaf { leaf_entries } => {
                if let Some((key, value)) = leaf_entries.next() {
                    ...
                    Some(TreeEntry { key, value })
                } else {
                    // If we've run out of entries in the leaf itself, we need to see if there is
                    // another valid leaf. We do this by changing state and recursing to use the
                    // existing logic.
