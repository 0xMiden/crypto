...
//! state of that tree.
//!
//! While any given [`Backend`] may choose to share data between lineages, his behaviour is not
...
impl<B: Backend> LargeSmtForest<B> {
    ...
    pub fn roots(&self) -> impl Iterator<Item = Root> {
        ...
        self.histories.keys().cloned().chain(
            self.histories
                .iter()
                .flat_map(|(k, v)| v.roots().map(|r| Root::new(k.domain(), r))),
        )
    }
