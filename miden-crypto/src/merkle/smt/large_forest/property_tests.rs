...
use crate::{
    ...
    merkle::smt::{
        Backend, ForestConfig, ForestInMemoryBackend, ForestOperation, LargeSmtForest, LeafIndex,
        LineageId, MAX_LEAF_ENTRIES, RootInfo, SMT_DEPTH, Smt, SmtUpdateBatch, TreeEntry, TreeId,
        VersionId,
    },
};
...
/// We reserve some headroom so property tests can safely add a few versions.
const MAX_START_VERSION: VersionId = u64::MAX - 8;
...
fn arbitrary_distinct_lineages() -> impl Strategy<Value = (LineageId, LineageId)> {
    (arbitrary_lineage(), arbitrary_lineage()).prop_filter("lineages must be distinct", |(a, b)| a != b)
}

/// Generates an arbitrary version identifier that leaves room for increments.
...
fn arbitrary_batch() -> impl Strategy<Value = SmtUpdateBatch> {
    arbitrary_entries().prop_map(|entries| {
        SmtUpdateBatch::new(entries.into_iter().map(|(k, v)| {
            if v == EMPTY_WORD {
...
fn apply_batch(tree: &mut Smt, batch: SmtUpdateBatch) -> core::result::Result<(), TestCaseError> {
    let mutations = tree.compute_mutations(Vec::<(Word, Word)>::from(batch).into_iter()).map_err(to_fail)?;
...
fn batch_keys(batch: &SmtUpdateBatch) -> Vec<Word> {
    Vec::<ForestOperation>::from(batch.clone()).into_iter().map(|operation| operation.key()).collect()
}
...
fn assert_lineage_metadata(
    ...
    let (latest_version, latest_root) = versions.last().copied().expect("lineage must be non-empty");
...
proptest! {
    ...
    fn entries_correct(
        ...
        let tree_info = forest.update_tree(lineage, version + 1, entries_v2.clone()).map_err(to_fail)?;
        ...
        prop_assert_eq!(sorted_forest_entries(&forest, old_version)?, sorted_tree_entries(&tree_v1));

        let current_version = TreeId::new(lineage, tree_info.version());
        prop_assert_eq!(sorted_forest_entries(&forest, current_version)?, sorted_tree_entries(&tree_v2));
    }
    ...
    fn queries_and_metadata_match_reference_model(
        ...
        let mut forest = LargeSmtForest::new(ForestInMemoryBackend::new()).map_err(to_fail)?;
        let add_result = forest.add_lineage(lineage, version, entries_v1.clone()).map_err(to_fail)?;
        let update_result = forest.update_tree(lineage, version + 1, entries_v2.clone()).map_err(to_fail)?;
        ...
        assert_tree_queries_match(&forest, TreeId::new(lineage, version), &tree_v1, &sample_keys, false)?;
    ...
    fn add_lineage_and_update_tree_preserve_state_on_failures(
        ...
        prop_assert_eq!(duplicate.unwrap_err().to_string(), format!("Duplicate lineage ID {lineage} provided"));
        assert_tree_queries_match(&forest, TreeId::new(lineage, version), &reference, &sample_keys, true)?;
        prop_assert_eq!(forest.lineage_count(), 1);
        ...
        let no_op = forest.update_tree(lineage, version + 1, SmtUpdateBatch::empty()).map_err(to_fail)?;
    ...
    fn update_forest_matches_reference_model_and_preserves_state_on_error(
        ...
        let mut forest_updates = crate::merkle::smt::SmtForestUpdateBatch::empty();
        forest_updates.add_operations(lineage_1, Vec::<ForestOperation>::from(updates_1.clone()).into_iter());
        forest_updates.add_operations(lineage_2, Vec::<ForestOperation>::from(updates_2.clone()).into_iter());
        ...
        let roots = forest.roots().map(|root| (root.lineage(), root.value())).sorted().collect_vec();
        let mut expected_roots = versions_1.iter().map(|(_, root)| (lineage_1, *root)).collect_vec();
        ...
        let mut invalid_updates = crate::merkle::smt::SmtForestUpdateBatch::empty();
        invalid_updates.add_operations(
            lineage_1,
            Vec::<ForestOperation>::from(SmtUpdateBatch::new([ForestOperation::insert(query_key, Word::new([ONE; 4]))].into_iter())).into_iter(),
        );
        invalid_updates.operations(unknown_lineage).add_insert(query_key, Word::new([ONE; 4]));
    ...
    fn new_loads_latest_backend_state_without_history(
        ...
        let mut expected_tree_1 = build_tree(entries_1.clone())?;
        apply_batch(&mut expected_tree_1, updates_1.clone())?;
        let expected_tree_2 = build_tree(entries_2.clone())?;
        let latest_version_1 = if expected_tree_1.root() == build_tree(entries_1.clone())?.root() {
            version
        } else {
            version + 1
        };
        ...
        assert_tree_queries_match(&forest, TreeId::new(lineage_1, latest_version_1), &expected_tree_1, &sample_keys, true)?;
        assert_tree_queries_match(&forest, TreeId::new(lineage_2, version), &expected_tree_2, &sample_keys, true)?;
        prop_assert_eq!(forest.lineage_count(), 2);
        ...
        prop_assert_eq!(forest.root_info(TreeId::new(lineage_1, version + 1)), expected_root_info);
    }
    ...
    fn with_config_and_truncate_limit_retained_versions(
        ...
        let mut forest = LargeSmtForest::with_config(ForestInMemoryBackend::new(), config).map_err(to_fail)?;
        forest.add_lineage(
            lineage,
            version,
        ...
        apply_batch(&mut tree_v1, SmtUpdateBatch::new([ForestOperation::insert(key_1, value_1)].into_iter()))?;
        let mut tree_v2 = tree_v1.clone();
        apply_batch(&mut tree_v2, SmtUpdateBatch::new([ForestOperation::insert(key_2, value_2)].into_iter()))?;
        let mut tree_v3 = tree_v2.clone();
        apply_batch(&mut tree_v3, SmtUpdateBatch::new([ForestOperation::insert(key_3, value_3)].into_iter()))?;
        let mut tree_v4 = tree_v3.clone();
        apply_batch(&mut tree_v4, SmtUpdateBatch::new([ForestOperation::insert(key_4, value_4)].into_iter()))?;

        let sample_keys = vec![key_1, key_2, key_3, key_4];
        assert_tree_queries_match(&forest, TreeId::new(lineage, version + 2), &tree_v3, &sample_keys, false)?;
        assert_tree_queries_match(&forest, TreeId::new(lineage, version + 3), &tree_v4, &sample_keys, true)?;
        prop_assert_eq!(forest.latest_version(lineage), Some(version + 3));
        prop_assert_eq!(forest.latest_root(lineage), Some(tree_v4.root()));
        prop_assert_eq!(forest.root_info(TreeId::new(lineage, version + 3)), RootInfo::LatestVersion(tree_v4.root()));
        prop_assert_eq!(forest.root_info(TreeId::new(lineage, version + 2)), RootInfo::HistoricalVersion(tree_v3.root()));
