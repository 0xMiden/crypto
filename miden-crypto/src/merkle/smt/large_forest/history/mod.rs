...
impl<'history> HistoryView<'history> {
    ...
    ///
    /// It is guaranteed to yield these entries in sorted order by key, and includes the key-value
    /// pairs where the value is the empty word, as these are explicit changes being made as part of
    /// the delta.
...
///
/// It yields entries as `(key, value)` pairs, in sorted order by key, and **includes** entries that
/// revert values for keys back to the empty word.
#[derive(Debug)]
...
impl<'history> HistoricalEntriesIterator<'history> {
    ...
    fn new(history: &'history History, version: VersionId) -> Self {
        // TODO This is ordered based on the leaf index which is a SUFFIX of the word.
        // TODO This makes it ordered wrong with relation to the actual words.
        // TODO So we have to have it ordered in the right way.
        //
        // TODO We can either store k-v changes by key and search to cohere into compact leaves or
        //   we can store auxiliary information to map between keys and populated leaves.
        //
        // TODO This yields a time/memory trade-off, and either keeps the iterator simpler or the
        //   leaf collection.
        //
        // TODO BTreeSet::range + storing the Words reversed?
        //
        // TODO to top this all off, iteration order for Smt::entries() is not only unspecified but
        //   crate feature dependent. I somehow have to merge things which needs a consistent order
        //   for the reason that Smt can remove entries with respect to the history, and the history
        //   can remove entries with respect to the Smt.
        //
        // TODO an alternative design—still predicated on some amount of sorting—is that we merge by
        //   leaf instead. This works if we assume we can get leaves in order and that each leaf is
        //   relatively small and can hence be ordered efficiently.
        ...
        match &mut self.position {
            HistoricalEntriesIteratorState::NotInLeaf => {
                ...
                if let Some(ix) = self.changed_leaves.pop_first() {
                    ...
                    self.position = HistoricalEntriesIteratorState::InLeaf {
                        index: ix,
                        value: leaf_delta,
                        key: item.key,
                    };
            ...
            HistoricalEntriesIteratorState::InLeaf { value, key, .. } => {
...
enum HistoricalEntriesIteratorState {
    ...
    InLeaf {
        /// The index of the compact leaf that the iterator is currently pointing to.
        index: LeafIndex<SMT_DEPTH>,

        /// The combined full delta that represents the compact leaf.
        value: CompactLeaf,

        /// The key in the compact leaf that the iterator is pointing to.
        key: Word,
    },
}
