...
use fjall::{Database, OwnedWriteBatch, Readable, Snapshot};
use lsm_tree;

use super::{StorageError, StorageResult, config::*, kvdb::*};
...
pub struct FjallKVDBSchemaConfig {
    pub db: Database,
    pub keyspaces: HashMap<&'static str, fjall::Keyspace>,
}
...
impl KVDB for FjallKVDB {
    ...
    fn new(
        ...
        schema_factory: impl FnOnce(
            ...
            Database,
        ) -> StorageResult<FjallKVDBSchemaConfig>,
    ) -> StorageResult<Self> {
        ...
        let FjallKVDBSchemaConfig { db, keyspaces } = schema_factory(config, db)?;
