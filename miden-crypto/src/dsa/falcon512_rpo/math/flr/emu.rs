...
impl FLR {
    ...
    pub(crate) fn decode(src: &[u8]) -> Option<Self> {
        match src.len() {
            8 => Some(Self(u64::from_le_bytes(*<&[u8; 8]>::try_from(src).unwrap()))),
            _ => None,
        }
    }
    ...
    pub(crate) fn set_mul(&mut self, other: Self) {
        ...
        //  - On some 64-bit platforms this is not constant-time (e.g. ARM Cortex-A53 and A55 CPUs).
        //  - On 32-bit platforms this might be done with a software routine that shortcuts cases
        //    when operands fit on 32 bits.
        //
    ...
    pub(crate) fn set_div(&mut self, other: Self) {
        ...
        // Aggregate scaling factor:
        //  - Each source exponent is biased by 1023, with an extra 52 for considering the mantissas
        //    as integers; these two bias cancel out each other.
        //  - The quotient was produced with a 55-bit scaling, but we may have removed one bit from
        //    that.
    ...
    // Miden-specific helpers for f64 conversion (for testing/debugging)
...
// impl DefaultIsZeroes for FLR { }  // Not available in miden-crypto
...
#[cfg(test)]
mod flr_cross_verification_tests {
    use super::*;

    /// Test that basic FLR arithmetic operations produce expected results.
    /// These test cases use values from the sampler KAT to ensure consistency
    /// with the fn-dsa reference implementation.
    #[test]
    fn test_flr_basic_operations() {
        // Test values that appear in the sampler KAT
        let test_pairs = [
            // (value, scale) pairs used in sampling
            (0x16f9e6cb3119a4i64, -52 + 6),
            (0x12c8142a489b3ci64, -52 - 1),
            (0x10a52739d97620i64, -52 + 3),
            // Additional test values
            (1i64 << 54, 0),    // Large positive value
            (-(1i64 << 54), 0), // Large negative value
            (12345i64, -10),
        ];

        for (val, exp) in test_pairs {
            let a = FLR::scaled(val, exp);
            let b = FLR::scaled(val, exp);

            // Test addition: a + a = 2a
            let sum = a + b;
            let expected_sum = FLR::scaled(val.saturating_mul(2), exp);
            // Allow small rounding differences due to mantissa precision
            assert!(
                (sum.to_f64() - expected_sum.to_f64()).abs()
                    < 1e-10 * expected_sum.to_f64().abs().max(1.0),
                "Addition failed for ({}, {}): got {}, expected {}",
                val,
                exp,
                sum.to_f64(),
                expected_sum.to_f64()
            ...
            let diff = a - b;
            assert_eq!(diff.0, 0, "a - a should be zero for ({}, {})", val, exp);

            // Test multiplication: a * 1 = a
            let one = FLR::from_i64(1);
            let prod = a * one;
            assert!(
                (prod.to_f64() - a.to_f64()).abs() < 1e-10 * a.to_f64().abs().max(1.0),
                "Multiplication by 1 failed for ({}, {})",
                val,
                exp
            );
            ...
            let neg = -a;
            let double_neg = -neg;
            assert_eq!(a.0, double_neg.0, "Double negation failed for ({}, {})", val, exp);
        }
    }

    /// Test FLR::expm_p63() with specific values from the sampler.
    /// This function is critical for ber_exp() and must match fn-dsa exactly.
    #[test]
    fn test_flr_expm_p63() {
        // Test with values from the first KAT sample iteration
        // Note: z_before_shift = (expm_p63() << 1) - 1, so we need to reverse this
        let test_cases = [
            // (r, ccs, z_before_shift) tuples from debug output
            (0.47400784645356381f64, 0.74999085326766490f64, 0x7784dcfd975af865u64),
            (0.15663911365244310f64, 0.74999085326766490f64, 0xa42910a9cc61043du64),
            (0.32565375358506254f64, 0.74999085326766490f64, 0x8aa205a5ae043279u64),
        ];

        for (r_f64, ccs_f64, z_before_shift) in test_cases {
            let r = FLR::from_f64(r_f64);
            let ccs = FLR::from_f64(ccs_f64);
            let result = r.expm_p63(ccs);

            // Compute what z_before_shift should be: (result << 1) - 1
            let computed_z = (result << 1).wrapping_sub(1);

            // Allow 1 ULP tolerance due to potential rounding differences
            let diff = if computed_z > z_before_shift {
                computed_z - z_before_shift
            } else {
                z_before_shift - computed_z
            };
            assert!(
                diff <= 1,
                "expm_p63({:.17}, {:.17}) produced z_before_shift=0x{:016x}, expected 0x{:016x} (diff={})",
                r_f64,
                ccs_f64,
                computed_z,
                z_before_shift,
    ...
    /// Test FLR::floor() and related operations
    #[test]
    fn test_flr_floor_trunc() {
        let test_cases = [
            (91.904712f64, 91i64),
            (-91.904712f64, -92i64),
            (0.5f64, 0i64),
        ...
        for (val_f64, expected_floor) in test_cases {
            let val = FLR::from_f64(val_f64);
            let floor_result = val.floor();
            assert_eq!(
                floor_result, expected_floor,
                "floor({}) = {}, expected {}",
                val_f64, floor_result, expected_floor
            );

            // trunc() should give integer part toward zero
            let trunc_result = val.trunc();
            let expected_trunc = val_f64.trunc() as i64;
            assert_eq!(
                trunc_result, expected_trunc,
                "trunc({}) = {}, expected {}",
                val_f64, trunc_result, expected_trunc
            );
        }
    }

    /// Test FLR::square() and FLR::half()
    #[test]
    fn test_flr_square_half() {
        let test_values = [2.0f64, 0.5f64, -3.0f64, 0.152712f64, 123.456f64];

        for val_f64 in test_values {
            let val = FLR::from_f64(val_f64);

            // Test square
            let sq = val.square();
            let expected_sq = val_f64 * val_f64;
            let rel_error = (sq.to_f64() - expected_sq).abs() / expected_sq.max(1e-10);
            assert!(
                rel_error < 1e-10,
                "square({}) = {}, expected {} (rel_error={})",
                val_f64,
                sq.to_f64(),
                expected_sq,
                rel_error
            ...
            // Test half (only for positive values)
            if val_f64 > 0.0 {
                let h = val.half();
                let expected_half = val_f64 / 2.0;
                let rel_error = (h.to_f64() - expected_half).abs() / expected_half;
                assert!(
                    rel_error < 1e-10,
                    "half({}) = {}, expected {} (rel_error={})",
                    val_f64,
                    h.to_f64(),
                    expected_half,
                    rel_error
    ...
    /// Test that FLR constants match expected values
    #[test]
    fn test_flr_constants() {
        // LOG2 = ln(2) ≈ 0.693147180559945309417232121458
        let log2_expected = core::f64::consts::LN_2;
        let log2_val = FLR::scaled(6243314768165359, -53);
        let log2_diff = (log2_val.to_f64() - log2_expected).abs();
        assert!(
            log2_diff < 1e-15,
            "LOG2 constant mismatch: got {}, expected {} (diff={})",
            log2_val.to_f64(),
            log2_expected,
            log2_diff
        );

        // INV_LOG2 = 1/ln(2) ≈ 1.442695040888963407359924681002
        let inv_log2_expected = core::f64::consts::LOG2_E;
        let inv_log2_val = FLR::scaled(6497320848556798, -52);
        let inv_log2_diff = (inv_log2_val.to_f64() - inv_log2_expected).abs();
        assert!(
            inv_log2_diff < 1e-14,
            "INV_LOG2 constant mismatch: got {}, expected {} (diff={})",
            inv_log2_val.to_f64(),
            inv_log2_expected,
            inv_log2_diff
        );
