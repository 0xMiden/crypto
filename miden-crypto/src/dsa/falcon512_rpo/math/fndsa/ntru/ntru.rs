...
pub(crate) fn solve_NTRU(
    ...
    for &tmp in tmp_u32.iter().take(2 * n) {
        let z = tmp as i32;
...
fn solve_NTRU_intermediate(
    ...
    tmp_u32.copy_within((2 * tlen * hn)..(2 * tlen * hn + 2 * slen * n), 2 * llen * n);
...
fn solve_NTRU_depth0(
...

    {
        ...
        for i in 0..n {
            ...
            let x = mp_sub(mp_mmul(t2[i], Gp[i], p, p0i), mp_mmul(t3[i], Fp[i], p, p0i), p);
            if x != rv {
                return false;
            }
        }
...
fn make_fg_step(logn_top: u32, depth: u32, work: &mut [u32]) {
    ...
    // inverse NTT as we go, so that we get the sources in RNS (non-NTT).
    {
        ...
        for (i, prime) in PRIMES.iter().enumerate().take(slen) {
            ...
            for j in 0..hn {
                fd[kd + j] =
                    mp_mmul(mp_mmul(fs[ks + 2 * j], fs[ks + 2 * j + 1], p, p0i), R2, p, p0i);
                gd[kd + j] =
                    mp_mmul(mp_mmul(gs[ks + 2 * j], gs[ks + 2 * j + 1], p, p0i), R2, p, p0i);
            }
