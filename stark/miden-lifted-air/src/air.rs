    ...
    EF: ExtensionField<F>,
{
    ...
    ///
    /// The default order is `air_inputs.len()`, `air_inputs`, `aux_inputs.len()`,
    /// then `aux_inputs`. The protocol observes the instance count and
    /// `log_trace_heights` separately after this hook, but passes the heights
