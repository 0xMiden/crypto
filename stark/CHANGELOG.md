...
- [BREAKING] Preprocessed (fixed) traces are now supported. AIRs declare their shape via `LiftedAir::preprocessed_width()` and provide content through `BaseAir::preprocessed_trace()`; the prover commits the preprocessed LDE tree once with `Preprocessed::build`, observes its commitment before per-proof statement data, and opens it as a PCS group before main, aux, and quotient traces.
...
- [BREAKING] `LiftedAir::log_quotient_degree` removed; quotient chunking is now the free function `miden_lifted_stark::log_quotient_degree`. `LiftedAir::constraint_degree` returns `ConstraintDegrees { base, ext }` (the raw base/extension symbolic-degree split, no clamping) instead of a `usize`; `log_quotient_degree` combines them and clamps the quotient degree to `D ≥ 1`, so trivial/degenerate AIRs are now supported rather than rejected (`AirStructureError::TrivialConstraints` is gone). The AIR ↔ PCS compatibility check (`log_quotient_degree ≤ log_blowup`) is inlined into the prover and verifier as `DomainError::ConstraintDegreeTooHigh`. ([#992](https://github.com/0xMiden/crypto/pull/992))
