...
- [BREAKING] Replaced `LiftedAir::reduced_aux_values` and `num_var_len_public_inputs` with a more general `eval_external` method. AIRs now express external assertions as a flat list of extension-field values that must equal zero, and `external_public_inputs: &[F]` is threaded through `AirInstance` / `AirWitness` and the prove/verify entry points; the AIR owns the schema for this slice and validates it inside `eval_external` ([#992](https://github.com/0xMiden/crypto/pull/992)).
- [BREAKING] Replaced per-AIR `LiftedAir::eval_external` with a new `ExternalEvaluator` trait that produces the proof's external assertions in a single global call across every AIR. `verify_multi`/`verify_single` take an `&E: ExternalEvaluator<F, EF>` argument; pass `NoExternalAssertions` when a proof has no cross-AIR external constraints. `AirInstance` and `AirWitness` moved from `miden-lifted-stark` to `miden-lifted-air` to sit alongside `AuxBuilder` and the new trait, but remain re-exported from `miden-lifted-stark`.
- [BREAKING] Moved external public inputs off `AirInstance` / `AirWitness` and onto the `ExternalEvaluator` (`external_public_inputs(&self) -> &[F]`). The prove/verify framework absorbs them into Fiat-Shamir automatically before instance shapes, so callers no longer pre-bind them. `prove_multi`/`prove_single` now also take an `&E: ExternalEvaluator<F, EF>` argument, and both `prove_single` and `verify_single` drop their `external_public_inputs: &[F]` parameter. `AirWitness::new` now takes only `(trace, public_values)`.
