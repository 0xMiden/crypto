...
- Added a wasm32+simd128 `PackedFelt` (a 2-lane SIMD-packed `Felt`) in `miden-field`, behind a new `packed-felt-simd128` cargo feature (default ON). Self-hosted using `core::arch::wasm32::*` intrinsics — no Plonky3 dependency. With the feature on AND target = wasm32+simd128, `<Felt as Field>::Packing` resolves to `PackedFelt` (`WIDTH = 2`); on every other target / feature configuration the cfg-gate makes it a literal no-op and `Felt::Packing = Self`. ([#998](https://github.com/0xMiden/crypto/pull/998)).
- Replaced the concrete `impl Permutation<[Felt; STATE_WIDTH]>` impls on `RpoPermutation256`, `RpxPermutation256`, and `Poseidon2Permutation256` in `miden-crypto` with a trait-generic blanket `impl<P: PackedValue<Value = Felt>> Permutation<[P; STATE_WIDTH]>`. The `P = Felt` (`WIDTH = 1`) case takes a const-folded fast path that's byte-equivalent to the previous concrete impl — verified via paired criterion benches in `miden-crypto/benches/hash.rs` (delta within timing noise). The blanket picks up any downstream packed type satisfying the bound (e.g. `miden-field`'s new `PackedFelt`, future Plonky3-shipped packed Goldilocks for any arch) automatically. ([#998](https://github.com/0xMiden/crypto/pull/998)).
- Re-exported `PackedValue` from `miden-field` (alongside the existing `Packable`, `Field`, etc. re-exports from `p3-field`) so consumers can import it via `miden_field::PackedValue`. ([#998](https://github.com/0xMiden/crypto/pull/998)).
